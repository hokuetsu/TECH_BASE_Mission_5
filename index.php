<?php
	...
	$pdo = new PDO($dsn, $user, $password, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_WARNING));
        ...
        if (isset($_FILES['upfile']['error']) && is_int($_FILES['upfile']['error']) && $_FILES["upfile"]["name"] !== ""){
            ...
            //画像・動画をDBに格納．
            $sql = "INSERT INTO DB_20190816 (fname, extension, raw_data) VALUES (:fname, :extension, :raw_data);";
...
<body>
    ...
    <?php
    //DBから取得して表示する．
    $sql = "SELECT * FROM DB_20190816 ORDER BY id;";
    $stmt = $pdo->prepare($sql);
    $stmt -> execute();
	...
	echo $row["id"]." ";
            echo ("<a href='import_media.php?target=$target'>ファイルを見る</a>");
        }
        echo ("<hr/>");
    }
    ?>
